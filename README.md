...
### Dodawanie algorytmów kryptografii postkwantowej

#### 1. Dodaj bibliotekę kryptograficzną

Przykład z **PQCrypto** lub **liboqs-python**:
...
#### 2. Stwórz moduł crypto

```python
# backend/crypto/__init__.py
# backend/crypto/kyber.py
from oqs import KeyEncapsulation

def kyber_generate_keypair():
    """Generuje parę kluczy Kyber"""
    kem = KeyEncapsulation("Kyber512")
    public_key = kem.generate_keypair()
    secret_key = kem.export_secret_key()
    return public_key, secret_key

def kyber_encrypt(public_key: bytes, message: bytes) -> tuple:
    """Szyfruje wiadomość kluczem publicznym Kyber"""
    kem = KeyEncapsulation("Kyber512")
    ciphertext, shared_secret = kem.encap_secret(public_key)
    # Użyj shared_secret do szyfrowania wiadomości (np. AES)
    return ciphertext, shared_secret

def kyber_decrypt(secret_key: bytes, ciphertext: bytes) -> bytes:
    """Deszyfruje wiadomość kluczem prywatnym Kyber"""
    kem = KeyEncapsulation("Kyber512")
    shared_secret = kem.decap_secret(ciphertext)
    return shared_secret
```

#### 3. Dodaj WebSocket events do backendu

```python
# backend/app.py

from crypto.kyber import kyber_generate_keypair, kyber_encrypt, kyber_decrypt
...
def handle_encrypt(data):
    ...
    ciphertext, shared_secret = kyber_encrypt(public_key, message.encode())
...
#### 4. Dodaj UI do frontendu

```typescript
// frontend/src/components/CryptoPanel.tsx
...
export function CryptoPanel() {
  ...
  const generateKeys = () => {
    socket.emit('generate_keys', { algorithm: 'kyber' })
  }

  socket.on('keys_generated', (data) => {
    setPublicKey(data.public_key)
    setSecretKey(data.secret_key)
  })

  const encryptMessage = () => {
    socket.emit('encrypt_message', {
      message,
  ...
  return (
    <div>
      <h2>🔐 Kryptografia Postkwantowa</h2>
      <button onClick={generateKeys}>Generuj klucze (Kyber512)</button>
      ...
      {encrypted && <div>Zaszyfrowano: {encrypted.substring(0, 50)}...</div>}
    </div>
  )
}
```
...
def test_kyber_encrypt_decrypt():
    """Test szyfrowania i deszyfrowania Kyber"""
    public_key, secret_key = kyber_generate_keypair()
    message = b"Test message"
    
    ciphertext, shared_secret_enc = kyber_encrypt(public_key, message)
    shared_secret_dec = kyber_decrypt(secret_key, ciphertext)
...
describe('Crypto Integration', () => {
  it('should generate Kyber keys', () => {
    const mockEmit = vi.spyOn(socket, 'emit')
    
    // Trigger key generation
    socket.emit('generate_keys', { algorithm: 'kyber' })
    
    expect(mockEmit).toHaveBeenCalledWith('generate_keys', {
      algorithm: 'kyber'
    })
  })
})
...
### Uruchomienie testów po zmianach
